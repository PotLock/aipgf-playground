...
return \`\${greating} World\`;`)
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          // Parse paths
          const paths = Object.entries(json.paths || {}).flatMap(([path, methods]: [string, any]) => {
  ...
  const fetchContractMethods = async (chain: string, network: string, address: string) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_METADATA_URL}/api/abi?chain=${chain}&network=${network}&account=${address}`,
      )
      if (!response.ok) {
  ...
  const fetchMethodDetails = async (methodName: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_METADATA_URL}/api/${endpoint}?account=${contractAddress}&network=${network.toLowerCase()}&methods=${methodName}`,
      )
      if (!response.ok) {
  ...
  const updateContractJsonInput = async (selectedMethods: string[] = []) => {
    ...
    const jsonData = {
      ...
      network,
      contractAddress,
      methods: selectedMethodsData,
    }
    console.log("Updated Contract JSON Input:", jsonData)
                                      ...
                                      <CardContent className="pt-6 space-y-4">
                                        ...
                                        <div>
                                          ...
                                          <Input
                                            ...
                                            value={param.name}
                                            onChange={(e) =>
                                              updateApiParameter(pathIndex, paramIndex, "name", e.target.value)
                                            }
                                        ...
                                        <div>
                                          ...
                                          <Select
                                            value={param.in}
                                            onValueChange={(value) =>
                                              updateApiParameter(pathIndex, paramIndex, "in", value)
                                            }
                                          >
                                        ...
                                        <div>
                                          <Label htmlFor={`param-description-${pathIndex}-${paramIndex}`}>
                                            Description
                                          </Label>
                                          <Input
                                            ...
                                            onChange={(e) =>
                                              updateApiParameter(pathIndex, paramIndex, "description", e.target.value)
                                            }
                                        ...
                                        <div className="flex items-center space-x-2">
                                          <Checkbox
                                            ...
                                            onCheckedChange={(checked) =>
                                              updateApiParameter(pathIndex, paramIndex, "required", checked)
                                            }
                                          />
                                          <Label
                                        ...
                                        <div>
                                          ...
                                          <Select
                                            value={param.type}
                                            onValueChange={(value) =>
                                              updateApiParameter(pathIndex, paramIndex, "type", value)
                                            }
                                          >
              ...
              <TabsContent value="file">
                <div className="space-y-4">
                  <div>
                    ...
                    <Input id="api-spec-file" type="file" accept=".json" onChange={handleApiSpecFileUpload} />
                  </div>
                  {apiSpecFile && (
                      ...
                      <div className="mt-4">
                        ...
                        {apiPaths.map((path, index) => (
                          <div key={index} className="border p-2 rounded mb-2">
                            ...
                            <p>
                              <strong>Method:</strong> {path.method.toUpperCase()}
                            </p>
                            <p>
                              <strong>Summary:</strong> {path.summary}
                            </p>
                            <p>
                              <strong>Operation ID:</strong> {path.operationId}
                            </p>
                          </div>
                        ))}
                      </div>
      ...
      <CardContent className="space-y-6">
        <Form action={action}>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p id="description-description" className="text-sm text-muted-foreground">
                Provide a brief description of your project or request
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-4">
            ...
            <p id="form-type-description" className="text-sm text-muted-foreground">
              Select the type of form you want to fill out
            </p>
          </div>
          ...
          <Input name="typeName" className="hidden" type="text" defaultValue={selectedForm || ""} />
          <Input name="data" className="hidden" type="text" defaultValue={data} />
          {children}
        </Form>
      </CardContent>
    </Card>
